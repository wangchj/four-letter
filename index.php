...
function init()
{
	...
	$('.key').click(function()
	{
		var guess = $(this).text().toLowerCase(); //String of length 1
		...
		for(var i = 0; i < words.length; i++)
		{
			...
			//Make mask (m) for this word
			var m = mask;
			for(var pos = 0; pos < word.length; pos++)
				if(word.charAt(pos) == guess)
					m = setCharAt(m, pos, guess);
			
			//Add word to one partition
			if(m in partitions) //Partition already exist
				partitions[m].push(word);
			else //Partition not exist yet, make new
			{
				var partition = new Array();
				partition.push(word); //Add word to new partition
				partitions[m] = partition; //Add partition to partitions
			}
		}

		//Choose a partition to be next list of possible words
		mask = choosePartition(partitions);
		words = partitions[mask];

		console.log('Chosen partition: ' + mask + ' ' + words.length);

		//Update game progress ui
		for(var i = 0; i < word.length; i++)
			if(mask.charAt(i) == guess)
				$(uimask[i]).text(guess.toUpperCase()).css('color',maskcolor);

		//Check for win
		if(mask.indexOf('-') === -1)
		{
			//Set key color and remove click event (off)
		...
		else if(mask.indexOf(guess) == -1)
		{
			...
			$('.key').each(function(index){
				if($(this).text().toLowerCase() === guess)
					$(this).removeAttr('style').addClass('keymiss').removeClass('key').off();
			});

			$(uichance[tries]).attr('src','dot_gray.png');
			console.log('miss' + ' Chances: ' + tries)
		}
		//Hit
		else
		{
...
/**
 * Algorithm to choose a partition to use for next set of possible words.
 * @param partitions An object that contains a set of partitions. This JavaScript represents an Hashmap where each property is a string and a key for each partition, which is a list of words. The following shows an example of the content where xxxx represents a random word:
 *     {'----':[xxxx, xxxx, ...], '-a--':[xxxx, ...], '--a-':[xxxx, ...], ...}
 * @return A string that represent the key (property name) of the chosen partition.
 */
function choosePartition(partitions)
{
	var top    = 4;   //How many candidates
	var thresh = 10;  //The min number of words to be considered a candidate
	var canSum = 0;   //The sum of all words in candidate partitions
	...
	for(var key in partitions)
	{
		if(!partitions.hasOwnProperty(key))
			continue;
		p.push({'mask':key, 'words':partitions[key]});
		console.log(key + ' ' + partitions[key].length);
	}

	//Sort partition array
	p.sort(function(a, b){
		if(a['words'].length > b['words'].length) return -1;
		else if(a['words'].length < b['words'].length) return 1;
		else return 0;
	});

	for(var i = 0; i < p.length && i < top; i++)
	{
		if(p[i]['words'].length < thresh)
			break;
		canSum += p[i]['words'].length;
	}

	if(canSum == 0)
		return p[0]['mask'];

	//Get a random number from 1 to canSum
	var rand = Math.floor((Math.random() * canSum) + 1);
	console.log('choosePartition(): canSum: ' + canSum + ' rand: ' + rand);

	var sum = 0;
	for(var i = 0; i < p.length && i < top; i++)
	{
		sum += p[i]['words'].length;
		if(sum >= rand)
			return p[i]['mask'];
	}
...
/**
 * Call for every new game.
 */
function reset()
{
	$(uichance).attr('src', 'dot_blue.png');

	tries = 15;
	mask = '----';
	misses = new Array();
	//Copy the entire lexicon to possible words
	words = lexicon.slice();
	$('.mask').css('color', bgcolor).text('--');
	$('.key').css('background-color', '001f1f');
}

/**
 * Replace the character at pos with ch.
 * @param str the String object.
 * @param pos the index position to replace.
 * @param ch  the character to replace with.
 */
function setCharAt(str, pos, ch) {
    if(pos > str.length - 1)
    	return str;
...
<body>
<center>
<h1>Four Letter Word</h1>
<span class="mask" id="mask1">S</span> <span class="mask" id="mask2">E</span> <span class="mask" id="mask3">A</span> <span class="mask" id="mask4">T</span>

<div class="progress">
		<img class="chance" src="dot_blue.png"/> <img  class="chance" src="dot_blue.png"/> <img  class="chance" src="dot_blue.png"/> <img  class="chance" src="dot_blue.png"/> <img  class="chance" src="dot_blue.png"/> <img  class="chance" src="dot_blue.png"/> <img  class="chance" src="dot_blue.png"/> <img  class="chance" src="dot_blue.png"/>
		 <img  class="chance" src="dot_blue.png"/> <img  class="chance" src="dot_blue.png"/> <img  class="chance" src="dot_blue.png"/> <img  class="chance" src="dot_blue.png"/> <img  class="chance" src="dot_blue.png"/> <img  class="chance" src="dot_blue.png"/> <img  class="chance" src="dot_blue.png"/>
</div>

<div id="keyboard">
	<div class="keyrow">
		<span class="key">Q</span> <span class="key">W</span> <span class="key">E</span> <span class="key">R</span> <span class="key">T</span> <span class="key">Y</span> <span class="key">U</span> <span class="key">I</span> <span class="key">O</span> <span class="key">P</span>
	</div>
	<div class="keyrow">
		<span class="key">A</span> <span class="key">S</span> <span class="key">D</span> <span class="key">F</span> <span class="key">G</span> <span class="key">H</span> <span class="key">J</span> <span class="key">K</span> <span class="key">L</span>
	</div>
	<div class="keyrow">
